...
import { buildMarketplaceArea } from "./scene/marketplace-area.js";
...
import { buildSaltwaterArea } from "./scene/saltwater-area.js";
import { isSaltwaterBuilding } from "./scene/saltwater-plan.js";
import { buildFireStationArea } from "./scene/fire-station-area.js";
import { isFireStationBuilding, isFireStationClearing } from "./scene/fire-station-plan.js";
import { buildPostOfficeArea } from "./scene/post-office-area.js";
...
// The figure the cast, the golfers and the deputy are all made of. Asked for
// here rather than on the first golfer, so nobody arrives as an empty space and
...
Promise.all([stairSpec, loadCabinAsset().catch(err => {
  ...
  .then(([stair, cabinAsset, fine, covers, near, osm, siteTrees, siteBoulders, siteShrubs,
    ...
    trees = buildTrees(scene, near.sample, near.cover, osm.roads, siteTrees, isFireStationClearing);
    ...
    saltwater = buildSaltwaterArea(scene, near.sample);
...
// The controls that drive the scene clock live in clock-control.js, which owns
// the hour, the day, the labels and the rule about what a past day costs. This
// is only the wiring: what to re-read when it moves, and what to hide.
const sceneClock = new ClockControl({
  onChange: () => {
    // Everything that reads the clock, re-read. The sun and the sky, the water
    // the shoreline runs at, the stream the drift rides, and the panels.
    updateSun();
...
function onPastDay() {
  return sceneClock.past;
}

// The oldest month the crossing figures reach is the oldest day anything here
// can say a true thing about.
function setClockLimits() {
  const months = feed.crossings && feed.crossings.data
    && feed.crossings.data.recent_months;
  sceneClock.setOldest(months && months.length
...
function frame() {
  ...
  if (saltwater) saltwater.update(level, camera, window.innerHeight, performance.now()/1000);
