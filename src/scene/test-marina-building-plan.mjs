...
assert.deepEqual(data.buildings.map((b,i)=>match(b)?i:null).filter(i=>i!==null),[0],"only the marina main building is replaced");
...
console.log(`PASS: exact one-building replacement, OSM corner alignment, world/local and aerial registration, waterside canopy and rooftop equipment. ${f.width.toFixed(2)} × ${f.length.toFixed(2)} m.`);
