...
export class Sky {
  constructor(scene) {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        uniform vec3 uSunDir; uniform float uCloud; uniform float uDay;
        void main() {
          float h = clamp(vDir.y, -0.1, 1.0);
          float t = pow(clamp(h, 0.0, 1.0), 0.6);
          vec3 col = mix(uHorizon, uTop, t);
          col *= (0.05 + 0.95 * uDay);            // darken toward night
          col += vec3(0.02, 0.03, 0.06) * (1.0 - uDay); // faint night blue
          ...
          float sun = (disk + glow) * (1.0 - 0.85 * uCloud) * uDay;
          col += uSunColor * sun;
  ...
  // daylight 0..1: 1 full day, 0 night.
  setDaylight(day) {
    this.uniforms.uDay.value = Math.max(0, Math.min(day, 1));
  }
