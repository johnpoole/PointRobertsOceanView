...
const {
  ACRE_M2, BUFFER_M, BUILDINGS, CLUSTER_STALLS, DEVELOPED_ACRES, FT,
  HEIGHT_CAP_M, HYDRANTS, PLINTH_M, SITES,
  distToRing, inRing, planCampground,
...
// the perimeter buffer. This is the whole point of buildableRect.
...
// The RVs come first, at the entrance, then the cabins, then the tents.
const order = plan.sites.map((s) => s.kind);
ok(order.indexOf("cabin") > order.lastIndexOf("rv"), "an RV is placed after a cabin");
ok(order.indexOf("tent") > order.lastIndexOf("cabin"), "a cabin is placed after a tent");
...
for (const s of plan.stalls) {
  near(s.x1 - s.x0, 9 * FT, 1e-6, "a stall's width");
  near(s.z1 - s.z0, 18 * FT, 1e-6, "a stall's length");
}
...
for (const c of cabins) {
  near(c.pad.x1 - c.pad.x0, 12 * FT, 1e-3, "a park model's width");
...
// The one number the rule cannot honour, reported rather than asserted away.
console.log(
  `lot ${plan.lotAcres} acres, developed ${plan.developedAcres.toFixed(2)}, ` +
  `${plan.sites.length} sites over ${plan.rungs} rungs, ` +
  `${plan.stalls.length} stalls, ${plan.buildings.length} buildings`);
console.log(
