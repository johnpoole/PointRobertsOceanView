...
The roofs are hipped, which is what both photographs show at both ends, so
`hipRoof` in `post-office-base.js` builds them: two slopes to a ridge along the
length and a hip off each end, one pitch throughout. The entrance bay is the
...
The floor is set at the door on the projecting bay, 15.15 m MLLW off the baked
...
- The aerial shows the north slope dark and the south slope light. That is read
  ...
  runs straight through and both photographs show one tile roof. If it turns out
...
every fitting on the front asserted to sit inside the wall it is fixed to.
